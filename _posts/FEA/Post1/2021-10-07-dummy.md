...
In the famous algebric equation given above, `[B]` is a gradient matrix and `[D]` is a constitutive matrix. `[B]` changes with element size.
In this case, we use linear material model for plane stress conditions.   
