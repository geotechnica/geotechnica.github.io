...
This result \\(670 \cdot 10^{-6} \mbox{in}\\) can be first approximation, `ANSYS` gives us \\(656\cdot 10^{-6} \mbox{in}\\) and finally `SymPy`'s calculation  \\(663.70\cdot 10^{-6} \mbox{in}\\). Indeed, Finite Element Methods are powerfull methods for approximation to exact solution. Note that, FE Methods can be dangerous in the wrong hand. Thus, I strongly recommend Daryl L. Logan's book in order to learn basis of FEA. 
