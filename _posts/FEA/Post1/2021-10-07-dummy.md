...
This result (670 \times 10^{-6}) can be first approximation, `ANSYS` gives us \\(0.656\cdot 10^{-6}\\) and finally `SymPy` calculate  \\(\sqrt{663.70^{2}+104.08^{2}} = 671.8 \cdot 10^{-6}\\)
